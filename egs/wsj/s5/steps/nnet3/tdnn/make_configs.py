...
def MakeConfigs(config_dir, splice_indexes_string,
    ...
    for i in range(0, num_hidden_layers):
        ...
        if xent_separate_forward_affine and i == num_hidden_layers - 1:
            ...
            if nonlin_type == "relu":
                if i >= 2 and i % 2 ==0:
                    prev_layer_output = nodes.AddResAffRelNormLayer(config_lines, "Tdnn_{0}".format(i), "Tdnn_{0}".format(i-2),
                                                            prev_layer_output, nonlin_output_dims[i-2], nonlin_output_dims[i],
                                                            self_repair_scale = self_repair_scale,
